    ...
    <div class="flex gap-4 overflow-x-auto pb-4">
        @foreach($statuses as $statusKey => $statusLabel)
            @php
                ...
                $columnStyles = [
                    'new'         => ['header' => 'bg-slate-50 border-slate-200',   'badge' => 'bg-slate-100 text-slate-700 border border-slate-200'],
                    'in_progress' => ['header' => 'bg-blue-50 border-blue-200',     'badge' => 'bg-blue-100 text-blue-700 border border-blue-200'],
                    'on_hold'     => ['header' => 'bg-amber-50 border-amber-200',   'badge' => 'bg-amber-100 text-amber-700 border border-amber-200'],
                    'to_close'    => ['header' => 'bg-violet-50 border-violet-200', 'badge' => 'bg-violet-100 text-violet-700 border border-violet-200'],
                    'closed'      => ['header' => 'bg-emerald-50 border-emerald-200','badge' => 'bg-emerald-100 text-emerald-700 border border-emerald-200'],
                ];
            ...
            <div class="flex-shrink-0 w-80">
                ...
                <div class="px-4 py-3 border rounded-t-lg {{ $style['header'] }}">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="ticket-count text-xs font-semibold px-2 py-1 rounded {{ $style['badge'] }}">
                            {{ $tickets->count() }}
                        </span>
                    </div>
                </div>
                ...
                <!-- Tickets Container -->
                <div
                    class="ticket-column bg-white border-l border-r border-b rounded-b-lg p-3 space-y-2.5 min-h-[200px] max-h-[calc(100vh-200px)] overflow-y-auto"
                ...
                >
                    @forelse($tickets as $ticket)
                        <div
                            ...
                            data-id="{{ $ticket->id }}"
                        >
                            <a href="{{ route('tickets.show', ['ticket' => $ticket, 'from' => 'status']) }}" class="block group">
    ...
    document.querySelectorAll('.ticket-column').forEach(column => {
        Sortable.create(column, {
            ...
            onEnd(event) {
                ...
                // Lege placeholder verbergen/tonen
